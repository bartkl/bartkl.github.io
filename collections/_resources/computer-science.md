---
title: Computer Science
layout: resource-list

list:
    talks and lectures:
        - title: The Next Paradigm Shift in Programming
          link: https://youtu.be/6YbK8o9rZfI
          author: Richard Feldman
          date: 2020-06-24
          tags: paradigms
        - title: Functional Design Patterns
          link: https://youtu.be/srQt1NAHYC0
          date: 2017-09-25
          author: Scott Wlaschin
          tags: fp
        - title: Don't Fear the Monad
          link: https://youtu.be/ZhuHCtR3xq8
          author: Brian Beckman
          date: 2021-11-21
          tags: fp
        - title: Source Decoded 3: Javascript -- Prototypes, Prototypal Inheritance done right.
          link: https://youtu.be/Yvf_kUBZmXg
          date: 2014-02-06
          author: Source Decoded
          description: A thorough discussion of JavaScripts prototypal foundations, with a critical glance on the
          increasing amount OOP concepts being introduced into the language.
          tags: js
    books:
        - title: Learn You a Haskell for Great Good!
          author: Miran Lipovača
          date: 2011-04
          tags: haskell fp
        - title: Eloquent JavaScript
          link: https://eloquentjavascript.net/
          author: Marijn Haverbeke
          date: 2018
          tags: js
    vlogs:
        - title: Object Oriented Programming vs Functional Programming
          link: https://youtu.be/wyABTfR9UTU 
          author: Dave Farley
          date: 2021-04-21
          tags: paradigms
    articles:
        - title: Scala Variance Explained
          link: https://lansalo.com/2017/10
          author: Lorenzo Ansaloni
          date: 2017-10
          tags: types
        - title: JavaScript Promises: an introduction
          link: https://web.dev/promises/
          author: Jake Archibald
          date: 2021-01-18
          tags: js
        - title: The Rise Of The State Machines
          link: https://www.smashingmagazine.com/2018/01/rise-state-machines/
          author: Krasimir Tsonev
          tags: 
    tools:
        - title: explainshell.com
          link: https://explainshell.com/
    people:
        - title: Doug Hellman
          link:
            - https://doughellmann.com/
            - https://twitter.com/doughellmann
          tags: python
        - title: Richard Feldman
          link: https://twitter.com/rtfeldman
          tags: fp elm
        - title: Scott Wlaschin
          link: https://twitter.com/ScottWlaschin
          tags: fp
---